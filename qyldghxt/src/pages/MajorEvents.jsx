...
const MajorEvents = () => {
  ...
  const loadEvents = async () => {
    const { responsible_department, ...rest } = filters
    const result = await getMajorEvents(rest)
    if (result.success) {
      let list = (result.data || []).map(e => {
        // 自动计算进度：实际成本 / 预算 * 100%
      ...
      if (filters.responsible_department) {
        const names = getDescendantDepartmentNames(departments, filters.responsible_department)
        if (names.length > 0) {
          list = list.filter(e => names.includes(e.responsible_department))
        }
      }
      setEvents(list)
    }
